...
client = Bot(description="Official SOUŁ々Bot is best", command_prefix="s!", pm_help = True)
...
async def on_ready():
    ...
    print('--------')
    print('--------')
    print('Started Official SOUŁ々Bot')
...
async def on_member_join(member):
    ...
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
    embed = discord.Embed(title=f'Welcome {member.name} to {member.server.name}', description='Do not forget to check <#506129177507004419> and never try to break any one of them', color = discord.Color((r << 16) + (g << 8) + b))
...
async def help(ctx):
    ...
    embed.add_field(name = 's!modhelp ',value ='Explaines all the commands which are only usable by Those who has moderation permissions. Like- Manage Nicknames, Manage Messages, Kick/Ban Members,etc.',inline = False)
    embed.add_field(name = 's!generalhelp ',value ='Explaines all the commands which are usable by everyone.',inline = False)
    await client.send_message(author,embed=embed)
...
async def modhelp(ctx):
    ...
    embed.set_image(url = 'https://image.ibb.co/caM2BK/help.gif')
    embed.add_field(name = 's!say(Admin permission required) ',value ='Use it like ``s!say <text>``',inline = False)
    embed.add_field(name = 's!embed(Admin permission required) ',value ='Use it like ``s!embed <text>``',inline = False)
    embed.add_field(name = 's!dm((Admin permission required) ',value ='Use it like ``s!dm @user <text>`` to dm anyone',inline = False)
    embed.add_field(name = 's!membercount(Kick members Permission Required) ',value ='Use it like ``s!membercount`` to get membercount',inline = False)
    embed.add_field(name = 's!removemod(Admin Permission Required)',value ='Use it like ``s!removemod @user`` to remove him from mod. Note-You need Moderator role in your server below darkbot to use it.',inline = False)
    embed.add_field(name = 's!makemod(Admin Permission Required)',value ='Use it like ``s!makemod @user`` to make him mod. Note-You need Moderator role in your server below darkbot to use it.',inline = False)
    embed.add_field(name = 's!setup(Admin Permission Required)',value ='Use it to add channels, voice channels and roles if your server is not developed currently and you have just 1-2 channels. Note- Use it only 1 time. If you will use same command again then it will do same thing again .i.e It will add true copy of previous channels + true copy of roles that made in previous command use. So be careful.',inline = False)
    embed.add_field(name = 's!friend(Admin Permission Required) ',value ='Use it like ``s!friend @user`` to give anyone Friend of Owner role',inline = False)
    embed.add_field(name = 's!role(Manage Roles Permission Required)',value ='Use it like ``s!role @user <rolename>``.',inline = False)
    embed.add_field(name = 's!setnick(Manage nickname permission required)',value ='Use it like ``s!setnick @user <New nickname>`` to change the nickname of tagged user.',inline = False)
    embed.add_field(name = 's!english(Kick members Permission Required)',value ='Use it like ``s!english @user`` when someone speaks languages other than English.',inline = False)
    embed.add_field(name = 's!serverinfo(Kick members Permission Required) ',value ='Use it like ``s!serverinfo`` to get server info',inline = False)
    embed.add_field(name = 's!userinfo(Kick members Permission Required) ',value ='Use it like ``s!userinfo @user`` to get some basic info of tagged user',inline = False)
    embed.add_field(name = 's!kick(Kick members Permission Required)',value ='Use it like ``s!kick @user`` to kick any user',inline = False)
    embed.add_field(name = 's!roles(Kick members Permission Required) ',value ='Use it to check roles present in server',inline = False)
    embed.add_field(name = 's!clear(Manage Messages Permission Required)',value ='Use it like ``s!clear <number>`` to clear any message',inline = False)
    embed.add_field(name = 's!mute(Mute members Permission Required)',value ='Use it like ``s!mute @user <time>`` to mute any user',inline = False)
    embed.add_field(name = 's!unmute(Mute members Permission Required) ',value ='Use it like ``s!unmute @user`` to unmute anyone',inline = False)
    embed.add_field(name = 's!ban(Ban members Permission Required) ',value ='Use it like ``s!ban @user`` to ban any user',inline = False)
    embed.add_field(name = 's!rules(Kick members Permission Required)',value ='Use it like ``s!rules @user <violation type>`` to warn user',inline = False)
    embed.add_field(name = 's!warn(Kick members Permission Required)',value ='Use it like ``s!warn @user <violation type>`` to warn any user',inline = False)    
    embed.add_field(name = 's!norole(Kick members Permission Required) ',value ='Use it like ``s!norole @user`` to warn anyone if he/she asks for promotion',inline = False)
    await client.send_message(author,embed=embed)
    await client.say('📨 Check DMs For Information')
...
async def generalhelp(ctx):
    ...
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
    embed = discord.Embed(color = discord.Color((r << 16) + (g << 8) + b))
    embed.add_field(name = 's!poll ',value ='Use it like ``s!poll "Question" "Option1" "Option2" ..... "Option9"``.',inline = False)
    embed.add_field(name = 's!guess ',value ='To play guess game use ``s!guess <number> and number should be between 1-10``',inline = False)
    embed.add_field(name = 's!github ',value ='Use it like- ``s!github uksoftworld/DarkBot``',inline = False)
    embed.add_field(name = 's!bottutorial ',value ='Use it like ``s!bottutorial <tutorial name by Sunny Singh>``',inline = False)
    embed.add_field(name = 's!dyno ',value ='Use it like ``s!s!dyno <dyno command name>``',inline = False)
    embed.add_field(name = 's!donate ',value ='Use it to donate us and get a special post on Official DarkBot server.',inline = False)
    embed.add_field(name = 's!ownerinfo ',value ='To get basic information about owner.',inline = False)
    embed.add_field(name = 's!sourcecode ',value ='Use it to see PuBg GuRu sourcecode.',inline = False)
    embed.add_field(name = 's!upvote ',value ='Use it to Upvote our bot and help us to grow',inline = False)
    embed.add_field(name = 's!authlink ',value ='Use it to get authorizing link to authorize this bot to your server.',inline = False)
    embed.add_field(name = 's!happybirthday @user ',value ='To wish someone happy birthday',inline = False)
    embed.add_field(name = 's!technews ',value ='Use it to get tech news',inline = False)
    embed.add_field(name = 's!googlefy ',value ='Use it like ``s!googlefy <string>``.',inline = False)
    embed.add_field(name = 's!spacenews ',value ='Use it to get space news',inline = False)
    embed.add_field(name = 's!phynews ',value ='Use it to get physycs',inline = False)
    embed.add_field(name = 's!verify ',value ='Use it to get verified role. Note- It needs proper setup.',inline = False)
    embed.add_field(name = 's!flipcoin ',value ='Flipps coin',inline = False)
    embed.add_field(name = 's!rolldice ',value ='Rolls dice',inline = False)
    embed.add_field(name = 's!avatar @user ',value ='Shows avatar',inline = False) 	
    await client.send_message(author,embed=embed)
    await client.say('📨 Check DMs For Information')
